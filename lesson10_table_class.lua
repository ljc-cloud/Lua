...
-- PrintName是表内部声明的，表内部声明的方法如果需要访问内部的变量和其他方法时，需要在添加第一个参数 
-- 然后再使用 表名:方法 的方式调用，这样默认就会将调用者作为第一个参数传入
Student:PrintName() -- 将Student作为参数传入

-- 表外部声明的方法，则不需要添加第一个参数，只需要 声明函数时 使用 function 表名:方法() end 即可
-- 这样方法内部就会有一个默认的参数self为调用者
...
-- .点 与 冒号: 的区别
-- 点. 调用表内部方法时，只是作为一个普通方法调用，不会将调用者传入
-- 冒号 : 调用表内方法时，会将调用者作为第一个参数传入